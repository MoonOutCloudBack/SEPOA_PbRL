...
class Workspace:
    ...
    def __init__(self, cfg):
        ...
        if cfg.use_wandb:
            exp_name = '_'.join([
                ...
                # "initAll" if cfg.agent.init_all else "",
            ])
    ...
    def train(self):
        ...
        if self.cfg.preference:
            ...
            if True:
                ...
                if self.skip_ratio_method == 'diff':
                    max_min_return = (np.max(averate_true_return_first) -
                                      np.min(averate_true_return_first)) * (self.cfg.segment_len / 1000)
        ...
        self.replay_buffer.add(time_step, meta, 0)
        if self.cfg.preference:
            self.pb_trainer.reward_model.skill_list.append(meta['task'])
        if self.cfg.save_train_video:
        ...
        while train_until_step(self.global_step):
            ...
            if not seed_until_step(self.global_step):
                ...
                if True:  # self.global_step >= self.cfg.num_interact:
                    if self.cfg.update_both_skill:
                        metrics = self.agent.update_without_skill_old(self.replay_iter, self.global_step)
                        metrics = self.agent.update(self.replay_iter, self.global_step)
                    else:
                        metrics = self.agent.update_without_skill_old(self.replay_iter, self.global_step)
                    self.logger.log_metrics(metrics, self.global_frame, ty='train')
...
def main(cfg):
    from finetune_pb_sd import Workspace as W
    root_dir = Path.cwd()
