...
class VideoRecorder:
    def __init__(self,
        ...
        if root_dir is not None:
            ...
            self.save_train_dir = root_dir / 'eval_video' / 'train_video'
            self.save_train_dir.mkdir(exist_ok=True)
        else:
            self.save_dir = None

        self.render_size = render_size
        self.is_quadruped = is_quadruped
    ...
    def record(self, env):
        if self.enabled:
            if hasattr(env, 'physics'):
                ...
                if self.is_quadruped:
                    # clip (480, 480, 3) to (112, 112, 3), both are np.ndarray
                    frame = frame[168: 168 + 112, 184: 184 + 112, :]
            else:
                frame = env.render()
